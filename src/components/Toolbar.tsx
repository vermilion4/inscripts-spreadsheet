...
const Toolbar = ({
  ...
  // Helper to build the actual columns array as used in SpreadsheetTable
  const getActualColumns = (
    activeSheet: SheetData | null | undefined
  ): { id: string; name: string; index: number }[] => {
    const baseColumns = [
      { id: 'row', name: 'Row', index: 0 },
      { id: 'job', name: 'Job Request', index: 1 },
      { id: 'submitted', name: 'Submitted', index: 2 },
      { id: 'status', name: 'Status', index: 3 },
      { id: 'submitter', name: 'Submitter', index: 4 },
      { id: 'url', name: 'URL', index: 5 },
      { id: 'assigned', name: 'Assigned', index: 6 },
      { id: 'priority', name: 'Priority', index: 7 },
      { id: 'due', name: 'Due Date', index: 8 },
      { id: 'value', name: 'Est. Value', index: 9 },
    ];
    const extraColumns = (activeSheet?.extraColumns || []).map(
      (col: { id: string; title: string }, idx: number) => ({
        id: col.id,
        name: col.title,
        index: 10 + idx,
      })
  ...
  const handleAddDynamicHeader = (header: DynamicHeader) => {
    const currentHeaders = activeSheet?.dynamicHeaders || [];
    const newHeaders = [...currentHeaders, header];
    onDynamicHeadersChange?.(newHeaders);
  };
  ...
  const importTemplates = rawImportTemplates.map(template => ({
    ...
    dynamicHeaders: (template.dynamicHeaders || []).map(header => ({
      ...
      columnSpans: Array.isArray(header.columnSpans)
        ? header.columnSpans.map(col =>
            typeof col === 'number' ? String(col) : col
          )
        : [],
    })),
  }));
  ...
  return (
    <>
      ...
      <NewActionModal
        ...
        onAddHeader={handleAddDynamicHeader}
